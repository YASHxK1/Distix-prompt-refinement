...
It uses your own Vercel AI Gateway API key and model. A prompt is processed only when you select **Refine**.
...
- Access to a model through Vercel AI Gateway
...
You can replace it with another Vercel AI Gateway model ID in the extension settings.
...
4. Enter a Gateway model ID in `provider/model` format.
5. Select **Save**.
6. Select **Test connection** to verify the key and model.
...
The connection test checks whether the saved key can access the selected model. It does not send a prompt.
...
Then select **Save** or **Test connection**. Testing also saves the values currently entered on the page.

The model setting is stored with Chrome Sync when browser sync is enabled. Your API key is stored only in Chrome's local extension storage on the current device.
...
- The trusted extension service worker sends the prompt and API key to Vercel AI Gateway.
- Vercel AI Gateway may route the request to the provider of your selected model.
- The extension does not automatically submit prompts to any website.

Use of Vercel AI Gateway and the selected model provider is subject to their respective terms and privacy policies. See [PRIVACY.md](PRIVACY.md) for the complete data-handling policy.

## Permissions

The extension requests only:

- **Storage:** saves the API key and selected model.
...
Open **Settings**, enter a valid Vercel AI Gateway API key, and select **Save**. The key must not contain spaces.
...
Check that the model uses the `provider/model` format and is available through your Vercel AI Gateway account.

### The budget is exhausted

Check the billing and budget limits associated with your Vercel AI Gateway account.

### The request timed out or the Gateway is unavailable

Check your internet connection and try again. Requests stop after 30 seconds. Temporary provider or Gateway outages may also cause this message.
...
Removing the extension clears its local extension data. The model ID may also exist in Chrome Sync and can be managed through Chrome's sync settings.
